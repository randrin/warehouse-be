...
public class WarehouseAuthService {
    ...
    public ResponseEntity<Object> registerUser(WarehouseRegisterRequest request) {
        ...
        if (userRepository.existsByUsername(request.getUsername())) {
            return ResponseEntity.badRequest().body(new WarehouseMessageResponse(WarehouseUserResponse.WAREHOUSE_USER_USERNAME_EXISTS + request.getUsername() + WarehouseUserResponse.WAREHOUSE_ERROR_ALREADY_USED));
        }

        if (userRepository.existsByEmail(request.getEmail())) {
            return ResponseEntity.badRequest().body(new WarehouseMessageResponse(WarehouseUserResponse.WAREHOUSE_USER_EMAIL_EXISTS + request.getEmail() + WarehouseUserResponse.WAREHOUSE_ERROR_ALREADY_USED));
        }
        ...
        return ResponseEntity.ok(new WarehouseMessageResponse(request.getUsername() + WarehouseUserResponse.WAREHOUSE_USER_REGISTERED));
    }
}
